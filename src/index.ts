...
const program = async (): Promise<void> => {
  const questionsData = await readJson('src/questions.json')();
  const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
  const ask = createAsk(rl);
  const answers = await askQuestions(ask)(questionsData.questions);
  rl.close();
  await writeJson('src/answers.json')(answers);
  console.log('✅ Réponses sauvegardées dans answers.json');
};

/** Lancement avec gestion de l'erreur */
program().catch((err) => console.error('❌ Erreur:', err));